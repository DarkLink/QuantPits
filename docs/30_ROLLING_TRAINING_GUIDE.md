...
`RollingEvidenceSetInspection`，逐 unit 核对 recorder/evidence identity，且最高只允许 `units_complete`。
...
Phase 34 先用 `bind_rolling_execution_run_identity()` 把实际观察的 business sessions 绑定到 runtime fingerprint；
随后 `build_rolling_execution_scope()`、`preflight_rolling_execution()`、
...
全部通过后才成为 `executed_success`。resume 只复用 State V2 冻结的 original attempt/recorder/evidence；不读取
