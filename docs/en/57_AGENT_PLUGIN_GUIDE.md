...
1. **Read Manifest**: Reads agent definitions from the file specified by the `--agent-manifest` CLI parameter (typically located at `workspaces/<name>/config/agent_manifest.json`). `--manifest` remains as a deprecated compatibility alias.
...
Specify the manifest configuration file using `--agent-manifest` and select the agent to run using `--agents`:
...
When debugging plugin loading or checkpoint reuse, add `--explain-plan` first. This mode prints the DAG, manifest loading result, upstream checkpoint hit/skip reasons, and the stages that would run, but it does not actually run the agent.
...
Loading uses the dedicated `--stage-manifest` parameter; agent plugins use `--agent-manifest`. Keeping the two manifest types separate prevents the agent and stage registries from misreading each other's configuration. See [50 — Deep Analysis Guide](50_DEEP_ANALYSIS_GUIDE.md) for details.

Before the first real run of a custom stage, use:
...
Confirm that the custom stage is registered, dependencies are correct, and upstream checkpoints are compatible. Then remove `--explain-plan` to execute the real pipeline.
