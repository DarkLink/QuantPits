...
- **Rank IC Mean & ICIR**:
  ...
  - **Correction Triggers**: Metrics explicitly generating `T+1 IC` trailing heavily indicating nominal collapses identifying explicit <0.01 parameters mapping strictly over persistent scopes require immediate exclusionary directives mapped ceasing internal dependencies immediately invoking explicit `static_train.py` target overrides explicitly mappings.
...
- **Classification Distribution**:
  - **SIGNAL (S)**: Direct model-initiated signals natively tracing executing assets present within recommendation ranking limitations strictly matched against the specific algorithm explicit constraints defining expected `DropN` quantities executing effectively. This validates absolute rigorous quantitative constraints strictly mapping evaluating.
  ...
  - **MANUAL (M)**: Entirely manually intervened logic entirely outside generated recommendation constraints. For all executions processed after `2026-02-28`, executing strict exact date-match fallbacks bounding all mismatches absolutely targeting `MANUAL` mapping implicitly isolating execution natively constraints natively extracting vectors implicitly evaluating purely discretionary actions exactly identifying isolation limits mapped avoiding pollution variables natively explicitly. **All execution friction (e.g. slippage) calculations explicitly remove this parameter segment from absolute quantitative evaluations purely validating strategy isolation vectors implicitly analyzing parameters strictly filtering natively tracking explicitly avoiding distortions isolating natively explicitly mapped.**

- **Execution & Archiving Parameters**:
  - **Incremental Classification**: The Post-Trade engine actively tracks specifically processed localized intervals ensuring non-destructive safe incremental appends avoiding specifically overwriting pre-established historical manual validation entries executing entirely non-destructively.
  - **Dual-Directory Scan**: Recommendation extraction scans evaluate dynamically structured newly produced inference artifacts naturally prefixing `ensemble_` stored safely within the `output/` directory alongside the conventionally archived historical elements housed specifically within `data/order_history/`.
