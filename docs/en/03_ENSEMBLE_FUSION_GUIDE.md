...
Implementation note: `ensemble_fusion.py` is now a thin CLI/process adapter. The argument contract and prepare/explain/execute routing live in the typed command runner in `quantpits/ensemble/command.py`. Plan rendering, manifest handling, OperatorLog linkage, and the execution lifecycle live in `quantpits/ensemble/service.py`; run-level execution context, prediction-loading boundaries, and expected execution errors live in `quantpits/ensemble/execution.py`. The engine layer does not call `sys.exit()`: the CLI adapter maps usage/plan errors to exit code 2 and maps no-executable-model or empty-filtered-prediction failures to exit code 1. Importing the script no longer changes the process `cwd`; during real execution, the service resolves relative paths such as `--output-dir` and `--prediction-dir` under the active workspace root. Single-combo Stage 2-10 orchestration lives in `quantpits/ensemble/pipeline.py`; prediction persistence, fusion ledger writes, deterministic analytics, Qlib backtest execution, risk/leaderboard reporting, and chart generation live in `quantpits/ensemble/persistence.py`, `quantpits/ensemble/ledger.py`, `quantpits/ensemble/analytics.py`, `quantpits/ensemble/backtest.py`, `quantpits/ensemble/risk_report.py`, and `quantpits/ensemble/charts.py`. The script keeps thin same-name wrappers for existing patch/import compatibility.

For programmatic use, prefer `quantpits.ensemble.command.run_ensemble_command()` with explicit `EnsembleCommandRequest` and `EnsembleCommandDependencies` values. It returns a structured outcome or raises a typed domain error without terminating the host process, making it suitable for tests, notebooks, schedulers, and future pipeline integrations.
