...
Developers can run `make test-semantic` (equivalent to `PYTHONDONTWRITEBYTECODE=1 python -m pytest tests/quantpits/semantic -q --tb=short --no-cov -p no:cacheprovider`) to verify identity, outcome, durable-effect, and recovery conservation across Training/Rolling, Ensemble, Post-trade, and Order. Its independent artifact observer records files, empty directories, symlinks, and physical containment, establishes the repository baseline before test collection imports test modules and production modules, and watches the complete repository tree except `.git` and the private `workspaces/` tree that tests are forbidden to read. Success, input failure, and publication failure each compare manifest, OperatorLog, and CLI mapping from the same real command/service invocation. The lane uses only pytest-generated sanitized workspaces and deterministic fakes at external boundaries. It does not read a real workspace, initialize real Qlib/MLflow, require network or GPU access, or replace the owner-operated full suite and real-backend smoke checks.
