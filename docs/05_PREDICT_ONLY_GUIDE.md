...
5. 持久化 publication intent，以可恢复 transaction 更新 record/performance，再写 verified receipt；失败 target 保留旧 current pointer
```
...
会逐模型区分 `published_this_attempt` / `already_published`，只把 receipt 验证的输出列为实际输出。
同身份 `--resume` 可以完成中断的 publication/manifest closure，不会重新预测已由 receipt 证明提交的 target。
部分成功已更新 current pointer 时，恢复身份仍采用 V3 state 记录的原始 source，不会把本次输出误当成新的输入。

TRA 使用的 Qlib `MTSDatasetH` 在内部读取 learn 数据。predict-only 构造该数据集时会在内存中的
配置副本里移除 `DropnaLabel`，避免未来标签尚不可见时把最新交易日从预测中删除；其他 learn
处理器、全部 infer 处理器及工作区 YAML 均保持不变。持久化预测仍须通过 requested anchor
覆盖校验，未覆盖锚点时不会发布该模型的新记录。
...
| `--dry-run` | - | 兼容别名：轻量计划，不初始化 Qlib/MLflow，不写文件 |
| `--explain-plan` | - | 人类可读轻量计划 |
| `--json-plan` | - | 单一 JSON 轻量计划 |
| `--workspace` | 当前 workspace | 显式 workspace 根目录 |
| `--run-id` | 自动生成 | 对齐 plan、manifest 与 OperatorLog |
| `--no-manifest` | false | 真实执行时不写 manifest |
...
合并前保存 transaction preimage；恢复依据是 durable intent/receipt，而不是时间戳备份。
...
5. **恢复证据**：每次发布都在 `data/training_transactions/` 保存 preimage/postimage 与 receipt
6. **TRA 末端日期**：predict-only 会保留 `MTSDatasetH` 中尚无未来标签的推理行；这些行用于生成预测，不代表未来标签已知
