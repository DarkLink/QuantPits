...
- **分类矩阵 (Classification Distribution)**：
  - **SIGNAL (S)**：模型主控信号。该笔交易的标的处于推荐列表的头部名额内（名额 = 当日基于配置算法严格推算的预期信号数 `DropN`）。这是严谨量化回测的基石。
  - **SUBSTITUTE (A)**：备用替代信号。该笔交易的标的在推荐列表中，但排名靠后（未进入当日前排）。通常发生在头部标的“一字涨停无法买入”或“停牌”等交易受阻引发的顺延动作，此分类可用于追溯因流动性妥协而付出的代价。
  - **MANUAL (M)**：完全手动操作。标的根本不在当期推荐列表中。对于 `2026-02-28` 之后的交易，如果当天完全没有相同日期的信号推荐文件，则当日所有操作一律被归谬为手动。**在随后的所有量化执行摩擦计算（滑点）中，该类交易将被全数剔除**。

- **运行与归档逻辑 (Execution & Archiving)**：
  - 分类引擎现在支持**增量执行 (Incremental Classification)**。在日常执行 `prod_post_trade.py` 时，分类引擎将只对当日新产生的交易进行溯源匹配，并安全地合并结果，**绝对不会覆盖或篡改任何你曾经进行过的历史人工校验或修正数据**。
  - 信号读取支持双源目录，默认能够识别当前尚未归档存放于 `output/` 目录下带有 `ensemble_` 融合前缀的动态生成推荐文件，以及归档在 `data/order_history/` 目录下的常规格式。
