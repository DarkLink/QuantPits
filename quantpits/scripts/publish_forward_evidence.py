"""Privacy-safe CLI for durable forward-definition evidence."""
...
def _parser() -> argparse.ArgumentParser:
    parser = _SafeParser(prog="publish_forward_evidence")
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    try:
        ...
        _emit(_blocked("ForwardDefinitionEvidenceArgumentError", "ARGUMENT_INVALID"))
        return 2
    try:
        ...
        if args.action == "preflight":
            result = prepare_forward_definition_evidence(
                args.workspace, args.evidence_cycle, args.activation,
                args.definition_store, args.evidence_store,
            )
            code = 0
        else:
            result = publish_forward_definition_evidence(
                args.workspace, args.evidence_cycle, args.activation,
                args.definition_store, args.evidence_store,
                args.expected_definition_set_id,
        ...
        _emit(_blocked("ForwardDefinitionEvidenceRuntimeError", "RUNTIME_BLOCKED"))
        return 2
    _emit(summary)
    return code
