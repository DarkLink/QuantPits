...
运行方式：
  ...
  # ICIR 加权
  python engine/scripts/ensemble_fusion.py --models gru,linear_Alpha158 --method icir_weighted

  # 手动权重
  python engine/scripts/ensemble_fusion.py --models gru,linear_Alpha158 --method manual --weights "gru:0.6,linear_Alpha158:0.4"

  # 动态权重（滚动 TopK Sharpe）
  python engine/scripts/ensemble_fusion.py --models gru,linear_Alpha158 --method dynamic

  # 从 ensemble_config.json 读取 default combo
  python engine/scripts/ensemble_fusion.py --from-config

  # 运行指定的 combo
  python engine/scripts/ensemble_fusion.py --combo combo_A

  # 运行所有 combo 并生成跨组合对比
  python engine/scripts/ensemble_fusion.py --from-config-all

  # 跳过回测
  python engine/scripts/ensemble_fusion.py --models gru,linear_Alpha158 --no-backtest
